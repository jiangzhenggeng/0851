	...
	public static function select_category($file = '',$catid = 0, $str = '', $default_option = '', $modelid = 0, $type = -1, $onlysub = 0,$siteid = 0,$is_push = 0) {
        ...
        if (is_array($result)) {
            foreach($result as $k=>$r) {
                if( $GLOBALS['keep_catid'] ){
                    if( !is_array($GLOBALS['keep_catid']) && strpos( ','.$result[$GLOBALS['keep_catid']]['arrchildid'].',',','.$r['catid'].',')===false ){
                        unset($result[$k]);
                    }else{
                        foreach ($GLOBALS['keep_catid'] as $k3 => $v3 ){
                            if( strpos( ','.$result[$v3]['arrchildid'].',',','.$r['catid'].',')===false ){
                                unset($result[$k]);
                            }
                        }
                    }
