...
class content extends admin {
	...
	public function init() {
		...
		if(isset($_GET['catid']) && $_GET['catid'] && $this->categorys[$_GET['catid']]['siteid']==$this->siteid) {
			...
			$where = 'catid in('.($category['arrchildid']?$category['arrchildid']:$catid).') AND status='.$status;
			//搜索
			...
			if(isset($_GET['shoolid']) && intval($_GET['shoolid'])>0) {
                $shoolid = intval($_GET['shoolid']);
                $where .= " AND `shoolid` = '$shoolid'";
            }
			...
			$tree->init($categorys);
				switch($from) {
					...
					default:
						...
						$strs2 = " <span class='folder'>\$catname  <a href='?m=content&c=content&a=\$type&menuid=".$_GET['menuid']."&catid=\$catid' target='right' onclick='open_list(this)'>列表</a></span>";
						break;
				}
