...
class content extends foreground {
	...
	public function publish() {
        ...
        $keep_catid = intval($_GET['keep_catid']?$_GET['keep_catid']:($_POST['keep_catid']?$_POST['keep_catid']:0));
		...
		if(isset($_POST['dosubmit'])) {
			...
			$id = $this->content_db->add_content($info,0,$keep_catid);
			//检查投稿奖励或扣除积分
			if ($info['status']==99) {
				$flag = $catid.'_'.$id;
            ...
            unset($forminfos['renqi']);
            unset($forminfos['hits']);
            unset($forminfos['islink']);
	...
	public function published() {
        ...
        $keep_catid = intval($_GET['keep_catid']?$_GET['keep_catid']:($_POST['keep_catid']?$_POST['keep_catid']:0));
		$infos = $this->content_check_db->listinfo(array('username'=>$_username, 'siteid'=>$siteid,'keep_catid'=>$keep_catid),'inputtime DESC',$page);
		$datas = array();
		foreach($infos as $_v) {
