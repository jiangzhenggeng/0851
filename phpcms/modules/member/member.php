...
class member extends admin {
	...
	public function apply_lists(){
        ...
        $db->table_name = $db->db_tablepre.'apply';
        $page = isset($_GET['page']) ? intval($_GET['page']) : 1;
        $apply_list = $db->listinfo('status=0', 'dataid DESC', $page, 15);

        $db->table_name = $db->db_tablepre.'shool';
        foreach ($apply_list as $k => $v ){
            $shool_data = $db->get_one('id='.$v['shoolid']);
            $apply_list[$k]['shool_catid'] = $shool_data['catid'];
        }
        $pages = $db->pages;
    ...
    public function apply_flage_unbind(){
        $db = pc_base::load_model('content_model');
        $db->table_name = $db->db_tablepre.'apply';
        $dataid = intval($_GET['dataid']);
        if($dataid<=0){
