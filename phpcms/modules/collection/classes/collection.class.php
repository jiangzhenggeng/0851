...
class collection {
	 ...
	 */
	public static function get_url_lists($url, &$config,$filter=false) {
		if ($html = self::get_html($url, $config)) {
            if($filter){
                $html = preg_replace('/\s+/',' ',strip_tags($html,'<a>'));
            }
	...
	protected static function cut_html($html, $start, $end) {
		...
		$end = trim(str_replace(array("\r", "\n"), "", $end));
		if($start){
            $html = explode($start, $html);
        }
		if(is_array($html) && $end){
            $html = explode($end, $html[1]);
        }
        $html = is_array($html)?trim($html[0]):$html;
		return $html;
	}
